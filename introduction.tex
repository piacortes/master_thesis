...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\columnwidth]{imagenes/exo_dischist_cumulative.png}
...
The Transit Timing Variations technique was proposed as a way to detect Earth-mass planet in multiplanetary systems due to gravitational interactions \citep{Holman2005,Agol2005}. A planet in a Keplerian orbit will transit its star following a linear function of the orbital period (Eq.~\ref{linear_ephemeris}), but the presence of a second planet could produce transits no exactly periodic, thus the time between two consecutive transits varies. 
...
In Equation .~\ref{linear_ephemeris}, $P$ is the orbital period and $E$ is an arbitrary epoch with the corresponding transit time $T_{c}(E)$. For a given light curve (see Figure \ref{transit}), the transit time is computed as:

\begin{equation}
T_{c} = \frac{t_T}{2}
\end{equation}

The variation in time produced by the perturbing body depends on the mass and geometry of their orbits. The magnitude of the variation between successive  transit is:

\begin{equation}
\Delta t = \frac{45\pi}{16} \frac{M_2}{M_*} P_{1} \alpha^3_{e} (1-	\sqrt{2}\alpha^{3/2})^{-2}
\end{equation}

Where $\alpha_e = \frac{a_1}{a_2(1-e_2)}$, $a_1$ and $P_1$ are the semi-major axis and orbital period of the transiting exoplanet, while the perturbing planet has a mass $M_2$, semi-major axis $a_2$  and orbital period $P_2$
